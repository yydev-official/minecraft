...
glm::mat4 player::get_view_matrix() const {
    return glm::lookAt(position, position + position.front().z_pos, position.upwards().y_pos);
}

void player::handle_input(GLFWwindow* window, float deltaTime, const world& my_world) {
    if (deltaTime > 0.1f) return;
    // 1. Check for crouch input (Left Control) - Note: if flying, control moves down instead!
    ...
    // Smoothly transition player height box size
    ...
    // Horizontal movement vectors[cite: 1]
    ...
    // Double-tap space detection for flight toggle (with a short cooldown to prevent accidental triggers)[cite: 1]
    ...
    if (is_flying) {
        ...
        // Only trigger jump on the exact frame space is newly pressed down
        ...
        // Cast a short ray downward from the bottom of the player
        glm::vec3 ray_origin = position - glm::vec3(0.0f, player_height, 0.0f);
        raycast_result ground_check = raycaster::cast(ray_origin, glm::vec3(0.0f, -1.0f, 0.0f), 0.2f, my_world);

        if (ground_check.hit) {
            is_grounded = true;
            velocity_y = 0.0f;
            // Snap cleanly to the top of the hit block
